...
@Service
public class FeedOzon implements Processing{
    ...
    private boolean downloadOzonAssortmentFile() {
        try {
            ftp.downloadFileFromFtp(ozonFtpDirectory, ozonItemsFile, false);
    ...
    private boolean downloadXlsLimitedFeed() {
        try {
            ftp.downloadFileFromFtp(xlsSourceFtpFolder, excelLimitedFeed, useCache);
