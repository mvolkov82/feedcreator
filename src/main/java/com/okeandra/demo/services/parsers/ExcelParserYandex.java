        ...
        try (HSSFWorkbook myExcelBook = new HSSFWorkbook(new FileInputStream(xlsFileName))) {
            ...
            for (int i = 0; i <= rowTotal; i++) {
                ...
                if (cellStockVendor != null) {
                    ...
                    // Минимальный остаток для Яндекса по складу Диденкова = 5 -> Алла, Телеграм от 1/02/2023. Из-за парфюма которого у поставщика 5 шт.
                    int stockDidenkov = (int) cellStockVendor.getNumericCellValue() - 4;
                    if (stockDidenkov < 0) {
                        stockDidenkov = 0;
